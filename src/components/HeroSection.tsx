        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="text-white">
           ...
           <div className="relative animate-slide-in-right">
             <div className="bg-white/10 backdrop-blur-md rounded-xl p-3 border border-white/20 max-w-sm">
               <h3 className="text-white text-base font-semibold mb-3 text-center flex items-center justify-center gap-2">
               ...
               <Carousel className="w-full">
                 <CarouselContent>
                   {featuredProducts.map((product) => (
                     <CarouselItem key={product.id}>
                       <div className="flex flex-col items-center text-center text-white p-2 space-y-2">
                         <div className="relative group">
                           <img 
                             src={product.imageUrl} 
                             alt={product.name}
                             className="w-24 h-24 object-cover rounded-lg mb-2 shadow-lg hover-lift transition-all duration-300 group-hover:scale-105"
                             onError={(e) => {
                               e.currentTarget.src = "https://images.unsplash.com/photo-1612815154858-60aa4c59eaa6?w=500&h=400&fit=crop";
                             }}
                           />
                           <div className="absolute -top-1 -right-1 bg-accent text-accent-foreground text-xs px-1 py-0.5 rounded-full font-bold animate-bounce-in">
                             TOP
                           </div>
                         </div>
                         ...
                         <div className="flex flex-wrap justify-center gap-1">
                           {product.features.slice(0, 2).map((feature, idx) => (
                             <span key={idx} className="text-xs bg-white/20 px-1.5 py-0.5 rounded-full backdrop-blur-sm">
                               {feature}
                             </span>
                           ))}
                         </div>
                         ...
                         <div className="flex gap-2 pt-1">
                           <Button 
                             size="sm" 
                             className="bg-white/20 hover:bg-white/30 text-white border border-white/30 text-xs px-2 py-1"
                             onClick={() => {
                               document.getElementById("catalogue")?.scrollIntoView({ behavior: "smooth" });
                             }}
                           >
                           ...
                           <Button 
                             size="sm" 
                             className="bg-accent hover:bg-accent/90 text-white text-xs px-2 py-1"
                             onClick={() => {
                               document.getElementById("devis")?.scrollIntoView({ behavior: "smooth" });
                             }}
                           >
                 ...
                 <CarouselPrevious className="left-1 bg-white/20 border-white/30 text-white hover:bg-white/30 h-6 w-6" />
                 <CarouselNext className="right-1 bg-white/20 border-white/30 text-white hover:bg-white/30 h-6 w-6" />
               </Carousel>
               
               {/* Indicateurs de popularité */}
               <div className="mt-2 text-center">
                 <p className="text-white/80 text-xs">
                   ⭐ Les choix préférés de nos clients
                 </p>
                 <div className="flex justify-center gap-1 mt-1">
                   {featuredProducts.map((_, index) => (
                     <div key={index} className="w-1 h-1 bg-accent rounded-full animate-pulse" style={{animationDelay: `${index * 0.2}s`}}></div>
                   ))}
                 </div>
               </div>
             </div>
